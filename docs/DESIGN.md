...
- **Phase 1(完了)**: 基盤構築 — Next.js/Prisma セットアップ、食品成分表マスタの取り込み
- **Phase 2(完了)**: テキストでの食事記録 + 栄養計算 + ダッシュボード + 日/週/月グラフ
- **Phase 3(完了)**: 写真解析(Claude Vision連携)+ 確認・補正UI
- **Phase 4(完了)**: 目標設定 + キャラクターアドバイザー + 食事・運動レコメンド
- **Phase 5**: 運動記録UIの充実、レコメンドロジックの精緻化
- **Phase 6**: iPhoneヘルスケア連携(ネイティブラッパー化)、Supabase Auth/Storageへの移行
